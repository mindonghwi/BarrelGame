...
void image::renderCenter(HDC hdc, int destX, int destY)
{
	if (_trans)
	{
		...
		GdiTransparentBlt(
			hdc,					
			destX - _imageInfo->width/2,
			destY - _imageInfo->height/2,
			_imageInfo->width,		
			_imageInfo->height,		
			_imageInfo->hMemDC,
			0, 0,					
	...
	{
		BitBlt(hdc, destX + _imageInfo->width / 2,
			destY + _imageInfo->height / 2,
			_imageInfo->width, _imageInfo->height,
			_imageInfo->hMemDC, 0, 0, SRCCOPY);
	}
...
void image::frameRenderCenter(HDC hdc, int destX, int destY, int currentFrameX, int currentFrameY)
{
	_imageInfo->currentFrameX = currentFrameX;
	_imageInfo->currentFrameY = currentFrameY;
	...
	{
		BitBlt(hdc, destX + _imageInfo->frameWidth / 2,
			destY + _imageInfo->frameHeight / 2,
			_imageInfo->frameWidth, _imageInfo->frameHeight,
			_imageInfo->hMemDC,
			_imageInfo->currentFrameX * _imageInfo->frameWidth,
...
int image::alphaRound(float fPosX, float fPosY, int nRadius)
{
	HBRUSH hB = CreateSolidBrush(Mins::getMazenta());
	HBRUSH hOb = (HBRUSH)SelectObject(_imageInfo->hMemDC, hB);
	EllipseMakeCenter(_imageInfo->hMemDC, fPosX, fPosY, nRadius * 2, nRadius * 2);
